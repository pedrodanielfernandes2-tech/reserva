...
export default function Page(){
  ...
  const [modalFotoSala,setModalFotoSala]=useState(null);
  ...
  // Tema escuro — carrega preferência salva
  useEffect(()=>{
    try{const s=localStorage.getItem("adl_tema");if(s==="escuro")setTemaEscuro(true);}catch(e){}
  },[]);

  // Aplica tema escuro no documento e sincroniza com iframe
  useEffect(()=>{
    try{document.documentElement.setAttribute("data-theme",temaEscuro?"dark":"light");}catch(e){}
    try{localStorage.setItem("adl_tema",temaEscuro?"escuro":"claro");}catch(e){}
    try{if(iframeRef.current?.contentWindow)iframeRef.current.contentWindow.postMessage({type:"adl_tema",escuro:temaEscuro},"*");}catch(e){}
  },[temaEscuro]);
  ...
  return(
    <>
      <header>
        ...
        <button onClick={()=>setTemaEscuro(t=>!t)} title={temaEscuro?"Modo claro":"Modo escuro"}
          style={{marginLeft:"auto",background:"none",border:"1.5px solid rgba(255,255,255,.35)",borderRadius:10,cursor:"pointer",color:"#fff",fontSize:20,padding:"6px 12px",lineHeight:1}}>
          {temaEscuro?"☀️":"🌙"}
        </button>
      </header>
        ...
        <main className="content" style={artesAberto?{padding:0,overflow:"hidden"}:secao==="home"?{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"calc(100vh - 80px)"}:{}}>
          ...
          {secao==="busca"&&reservasAberto&&!artesAberto&&(
            <div className="block" style={{marginTop:0}}>
              ...
              <p className="block-sub">Pesquise em todas as reservas por nome, evento ou sala.</p>
              <div className="form-field" style={{margin:"16px 0"}}>
                <input type="text" placeholder="Buscar por nome, evento ou sala…" value={buscaGeral}
                  onChange={e=>setBuscaGeral(e.target.value)} style={{fontSize:15,padding:"12px 16px"}} autoFocus/>
              </div>
              {buscaGeral.trim().length>=2?(()=>{
                const termo=buscaGeral.toLowerCase();
                const resultados=reservas.filter(r=>r.nome.toLowerCase().includes(termo)||r.evento.toLowerCase().includes(termo)||r.sala_nome.toLowerCase().includes(termo))
                  .sort((a,b)=>new Date(b.ano,b.mes,b.dia)-new Date(a.ano,a.mes,a.dia));
                return(
                  <div>
                    <p style={{fontSize:13,color:"var(--ink-soft)",marginBottom:12}}>{resultados.length} resultado(s)</p>
                    {resultados.length===0?<div className="empty-state">Nenhuma reserva encontrada.</div>:(
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                          ...
                          {resultados.map(r=>{
                            ...
                            return(
                              <tr key={r.id} style={{borderTop:"1px solid var(--border)",opacity:passada?0.6:1}}>
                                <td style={{padding:"8px 10px"}}><span style={{background:salas.find(s=>s.nome===r.sala_nome)?.cor||"#999",color:"#fff",padding:"2px 8px",borderRadius:6,fontSize:12,fontWeight:700}}>{r.sala_nome}</span></td>
                                ...
                                <td style={{padding:"8px 10px",fontWeight:600}}>{r.evento}</td>
                ...
                <div className="room-tabs">
                  {salas.map(s=>(
                    <button key={s.id} className={"room-tab"+(s.nome===salaAtiva?" active":"")} style={{"--tab-color":s.cor}} onClick={()=>setSalaAtiva(s.nome)}>
                      ...
                      {s.foto_url&&<span onClick={e=>{e.stopPropagation();setModalFotoSala({nome:s.nome,foto_url:s.foto_url});}} style={{cursor:"pointer",fontSize:13,marginLeft:2}} title="Ver foto">📷</span>}
                    </button>
                  ))}
                </div>
              )}
                      ...
                      <div className="form-grid">
                        ...
                        <div className="form-field">
                          ...
                          <div style={{display:"flex",alignItems:"center",gap:10}}>
                            <input type="color" value={formSala.cor||"#0E8E89"} onChange={e=>setFormSala(f=>({...f,cor:e.target.value}))} style={{width:44,height:40,borderRadius:8,border:"1.5px solid var(--border)",cursor:"pointer",padding:2}}/>
                            <span style={{fontSize:12,color:"var(--ink-soft)"}}>Cor no calendário</span>
                          </div>
                        </div>
                        <div className="form-field">
                      ...
                      <div className="sala-chip-list" style={{marginTop:14}}>
                        {salas.map((s,idx)=>(
                          <div className="sala-chip" key={s.id}>
                            {/* Color picker inline */}
                            <input type="color" value={s.cor||"#0E8E89"} title="Clique para mudar a cor"
                              onChange={async e=>{
                            ...
                            <div className="info" style={{flex:1}}>
                              ...
                              <input type="url" placeholder="URL da foto (opcional)" defaultValue={s.foto_url||""}
                                onBlur={async e=>{
                                ...
                                style={{fontSize:11,padding:"3px 7px",borderRadius:6,border:"1px solid var(--border)",background:"var(--surface-soft)",color:"var(--ink-soft)",marginTop:4,width:"100%"}}/>
                            </div>
      ...
      {/* MODAL FOTO DA SALA */}
      {modalFotoSala&&(()=>{
        // Converte URLs do Google Drive para URL de imagem direta
        ...
        const imgUrl = resolverUrl(modalFotoSala.foto_url);
        return(
          <div className="overlay show" onClick={()=>setModalFotoSala(null)}>
            <div className="modal" style={{maxWidth:580,padding:0,overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
              <div className="modal-head" style={{padding:"14px 18px"}}>
                <h3>📷 {modalFotoSala.nome}</h3>
                <button onClick={()=>setModalFotoSala(null)}>✕</button>
              </div>
              <div style={{width:"100%",background:"var(--surface-soft)",display:"flex",alignItems:"center",justifyContent:"center",minHeight:200,position:"relative"}}>
                <img src={imgUrl} alt={modalFotoSala.nome}
                  style={{width:"100%",maxHeight:440,objectFit:"contain",display:"block"}}
                  onError={e=>{
                    e.target.style.display="none";
                    e.target.nextSibling.style.display="flex";
                  }}/>
                <div style={{display:"none",flexDirection:"column",alignItems:"center",gap:10,padding:30,color:"var(--ink-soft)"}}>
                  <span style={{fontSize:40}}>🖼️</span>
                  <span style={{fontSize:13,textAlign:"center"}}>Não foi possível carregar a imagem.<br/>Verifique se o link está público.</span>
                  <a href={modalFotoSala.foto_url} target="_blank" rel="noreferrer"
                    style={{fontSize:12,color:"var(--primary)",fontWeight:700}}>Abrir link original ↗</a>
                </div>
              </div>
              <div style={{padding:"12px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <a href={imgUrl} target="_blank" rel="noreferrer"
                  style={{fontSize:12,color:"var(--ink-soft)",textDecoration:"none"}}>🔗 Abrir em nova aba</a>
                <button className="btn-primary" style={{gridColumn:"auto",padding:"8px 20px"}} onClick={()=>setModalFotoSala(null)}>Fechar</button>
              </div>
            </div>
          </div>
        );
