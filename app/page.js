...
export default function Page(){
  ...
  const [eventosPreview, setEventosPreview] = useState(null); // array para revisar antes de salvar
  const [salvandoEventos, setSalvandoEventos] = useState(false);

  // ---- CARREGAMENTO ----
  const carregarSalas = useCallback(async()=>{
    try{ const res=await fetch("/api/salas"); const d=await res.json(); setSalas(Array.isArray(d)?d:[]); setSalaAtiva(prev=>prev||(Array.isArray(d)&&d[0]?d[0].nome:null)); }catch(e){}
  },[]);
  const carregarReservas = useCallback(async()=>{
    try{ const res=await fetch("/api/reservas"); const d=await res.json(); setReservas(Array.isArray(d)?d:[]); }catch(e){}
  },[]);
  const carregarBloqueios = useCallback(async()=>{
    try{ const res=await fetch("/api/bloqueios"); const d=await res.json(); setBloqueios(Array.isArray(d)?d:[]); }catch(e){}
  },[]);
  const carregarEventosIgreja = useCallback(async()=>{
    try{ const res=await fetch("/api/eventos-igreja"); const d=await res.json(); setEventosIgreja(Array.isArray(d)?d:[]); }catch(e){}
  },[]);
  const carregarConfig = useCallback(async()=>{
    try{ const res=await fetch("/api/config"); const d=await res.json(); if(d&&typeof d==="object") setConfig(d); }catch(e){}
  },[]);

  useEffect(()=>{
    setVersiculo(VERSICULOS[Math.floor(Math.random()*VERSICULOS.length)]);
    (async()=>{
      try{ await Promise.all([carregarSalas(),carregarReservas(),carregarBloqueios(),carregarEventosIgreja(),carregarConfig()]);
  ...
  useEffect(()=>{ const t=setInterval(carregarReservas,15000); return()=>clearInterval(t); },[carregarReservas]);
  ...
  function abrirModal(dia){
    ...
    if(Math.round((dataEsc-hoje())/86400000)>LIMITE){ alert(`Só é possível reservar com até ${LIMITE} dias de antecedência.`); return; }
  ...
  async function enviarReserva(e){
    ...
    try{
      const res=await fetch("/api/reservas",{ method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ nome:nome.trim(),sala,evento:evento.trim(),observacao:observacao.trim(),dia:diaSelecionado,mes:mesAtual,ano:anoAtual,horaInicio,horaFim,recorrencia,recorrenciaFim,precisaSom,precisaProjecao }),
      });
  ...
  async function cadastrarSala(){
    setErroSala(""); if(!formSala.nome.trim()){ setErroSala("Informe um nome."); return; }
    const res=await fetch("/api/salas",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(formSala)});
    const d=await res.json(); if(!res.ok){ setErroSala(d.erro||"Erro ao cadastrar."); return; }
  ...
  async function cadastrarBloqueio(){
    setErroBloqueio(""); if(!formBloqueio.sala_nome||!formBloqueio.hora_inicio||!formBloqueio.hora_fim){ setErroBloqueio("Preencha todos os campos."); return; }
  ...
  // ---- EVENTOS IGREJA ----
  async function excluirEventoIgreja(id){
    await fetch(`/api/eventos-igreja/${id}`,{method:"DELETE"}); await carregarEventosIgreja();
  }
  ...
  async function importarPDF(e){
    ...
    try{
      ...
      const ano = anoAtual;
      const resp = await fetch("/api/eventos-igreja/importar",{ method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({pdfBase64:b64,ano}) });
  ...
  async function confirmarImportacao(){
    if(!eventosPreview?.length) return;
    setSalvandoEventos(true);
    try{
      const res=await fetch("/api/eventos-igreja",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(eventosPreview)});
      if(res.ok){ setEventosPreview(null); await carregarEventosIgreja(); }
  ...
  async function salvarConfig(){
    ...
    try{ const res=await fetch("/api/config",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(config)}); if(res.ok){ setSucessoConfig(true); setTimeout(()=>setSucessoConfig(false),3000); } }
  ...
  const reservasDoDia=reservas.filter(r=>r.dia===diaRel&&r.mes===mesRel-1&&r.ano===anoRel)
  ...
  function getEventosDia(dia,mes,ano){
    return eventosIgreja.filter(e=>e.dia===dia&&e.mes===mes&&e.ano===ano);
  }

  function TrashBtn({onClick}){
    return(
      <button type="button" onClick={onClick} title="Excluir"
        style={{background:"none",border:"none",cursor:"pointer",padding:"2px 4px",color:"#D6483A",fontSize:14,lineHeight:1,flexShrink:0}}>
        🗑️
      </button>
    );
  }
        ...
        <main className="content">
          ...
          {secao==="calendario"&&(
            <div className="block">
              ...
              {/* Legenda */}
              <div style={{display:"flex",gap:14,flexWrap:"wrap",margin:"12px 0 4px",fontSize:12,alignItems:"center"}}>
                <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"#D6483A",display:"inline-block"}}/>Evento Sede (bloqueia)</span>
                <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"#E0A23B",display:"inline-block"}}/>Evento Congregação (aviso)</span>
                <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:10,height:10,borderRadius:2,background:"#3F7FEA",display:"inline-block"}}/>Reserva</span>
              </div>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:salas.length*120+80}}>
                    ...
                    <tbody>
                      {diasDoMes.map(dia=>{
                        ...
                        const evsDia=getEventosDia(dia,mesAtual,anoAtual);
                        return(
                            ...
                            {salas.map(s=>{
                              ...
                              return(
                                <td key={s.id} style={{padding:"4px",textAlign:"center",verticalAlign:"top"}} onClick={()=>!passado&&!acima&&(setSalaAtiva(s.nome),abrirModal(dia))}>
                                  ...
                                  {bloqDia.map(b=><div key={b.id} style={{background:"#888",color:"#fff",borderRadius:4,padding:"2px 4px",fontSize:9,marginBottom:2}}>🚫 {b.hora_inicio}-{b.hora_fim}</div>)}
                                  {resv.map(r=>(
                                    <div key={r.id} style={{background:s.cor,color:"#fff",borderRadius:6,padding:"3px 5px",fontSize:10,fontWeight:700,marginBottom:2,display:"flex",alignItems:"center",justifyContent:"space-between",gap:2}}>
                                      ...
                                      {adminMode&&<TrashBtn onClick={e=>{e.stopPropagation();excluirReserva(r,true);}}/>}
                                    </div>
                                  ))}
                  ...
                  {diasDoMes.map(dia=>{
                    ...
                    return(
                      <div key={dia} className={"day"+(ehHoje?" today":"")+(acima?" fora-limite":"")}
                        ...
                        {bloqDia.map(b=><div key={b.id} className="reserva-item" style={{background:"#888",opacity:0.85,fontSize:"9.5px"}}>🚫 {b.hora_inicio}-{b.hora_fim}</div>)}
                        {resv.map(r=>(
                  ...
                  return(
                    <li key={r.id}>
                      ...
                      <div className="res-actions" style={{display:"flex",gap:6,alignItems:"center"}}>
                        ...
                        <button onClick={()=>excluirReserva(r,false)}>Excluir</button>
                      </div>
                    </li>
                  );
                })}
            ...
            <div className="block">
              ...
              {!adminMode?(
                ...
                <div style={{maxWidth:720}}>
                  ...
                  <h4 style={{fontFamily:"Fraunces,serif",color:"var(--primary-dark)",fontSize:16,margin:"32px 0 6px"}}>📅 Calendário da Igreja (PDF)</h4>
                  <p className="block-sub" style={{marginBottom:14}}>Importe o calendário anual em PDF — a IA extrai os eventos automaticamente, separa os da Sede (🔴 bloqueia reservas) dos de Congregações (🟡 aviso informativo).</p>
                  ...
                  <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:16,flexWrap:"wrap"}}>
                    <label style={{display:"inline-flex",alignItems:"center",gap:8,padding:"10px 16px",background:"var(--primary)",color:"#fff",borderRadius:11,fontWeight:700,fontSize:13.5,cursor:"pointer"}}>
                      {pdfImportando?"⏳ Processando PDF…":"📄 Importar PDF do Calendário"}
                      <input type="file" accept=".pdf" style={{display:"none"}} disabled={pdfImportando} onChange={importarPDF}/>
                    </label>
                    <small style={{color:"var(--ink-soft)",fontSize:12}}>Requer ANTHROPIC_API_KEY configurada na Vercel.</small>
                  </div>

                  {/* PREVIEW DOS EVENTOS EXTRAÍDOS */}
                  {eventosPreview&&(
                      ...
                      <div style={{maxHeight:320,overflowY:"auto",border:"1px solid var(--border)",borderRadius:12}}>
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                          <thead style={{position:"sticky",top:0,background:"var(--surface-soft)"}}>
                            <tr>
                              <th style={{padding:"8px 10px",textAlign:"left",fontWeight:700}}>Evento</th>
                              ...
                              <th style={{padding:"8px 10px",textAlign:"left",fontWeight:700}}>Congregação</th>
                              <th style={{padding:"8px 10px",textAlign:"left",fontWeight:700}}>Horário</th>
                              <th style={{padding:"8px 4px"}}></th>
                            </tr>
                          </thead>
                          <tbody>
                            {eventosPreview.map((ev,i)=>(
                              <tr key={i} style={{borderTop:"1px solid var(--border)",background:ev.tipo==="sede"?"#FFF5F5":ev.tipo==="congregacao"?"#FFFBF0":"#fff"}}>
                                <td style={{padding:"7px 10px"}}>{ev.tipo==="sede"?"🔴":"🟡"} {ev.nome}</td>
                                <td style={{padding:"7px 10px"}}>{String(ev.dia).padStart(2,"0")}/{String(ev.mes+1).padStart(2,"0")}/{ev.ano}</td>
                                <td style={{padding:"7px 10px"}}>
                                  <select value={ev.tipo} onChange={e=>{ const arr=[...eventosPreview]; arr[i]={...arr[i],tipo:e.target.value}; setEventosPreview(arr); }}
                                    style={{padding:"3px 6px",fontSize:12,borderRadius:6,border:"1px solid var(--border)"}}>
                                    <option value="sede">Sede</option>
                                    <option value="congregacao">Congregação</option>
                                  </select>
                                </td>
                                <td style={{padding:"7px 10px"}}>
                                  <input type="text" value={ev.congregacao||""} onChange={e=>{ const arr=[...eventosPreview]; arr[i]={...arr[i],congregacao:e.target.value}; setEventosPreview(arr); }}
                                    style={{width:90,padding:"3px 6px",fontSize:12,borderRadius:6,border:"1px solid var(--border)"}} placeholder="—"/>
                                </td>
                                <td style={{padding:"7px 10px",whiteSpace:"nowrap"}}>{ev.hora_inicio}–{ev.hora_fim}</td>
                                <td style={{padding:"7px 4px"}}>
                                  <button type="button" onClick={()=>setEventosPreview(eventosPreview.filter((_,j)=>j!==i))} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,color:"#D6483A"}}>🗑️</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <tbody>
                          {eventosIgreja.map(ev=>(
                            <tr key={ev.id} style={{borderTop:"1px solid var(--border)"}}>
                              ...
                              <td style={{padding:"6px 10px"}}>{String(ev.dia).padStart(2,"0")}/{String(ev.mes+1).padStart(2,"0")}/{ev.ano}</td>
                              <td style={{padding:"6px 10px"}}>{ev.tipo==="sede"?"Sede":`Congregação: ${ev.congregacao||"?"}`}</td>
                              <td style={{padding:"6px 4px"}}><TrashBtn onClick={()=>excluirEventoIgreja(ev.id)}/></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  {/* SALAS */}
                  <h4 style={{fontFamily:"Fraunces,serif",color:"var(--primary-dark)",fontSize:16,margin:"32px 0 12px"}}>🏛️ Salas e Nave</h4>
                  ...
                  <div className="sala-chip-list" style={{marginTop:14,marginBottom:24}}>
                    {salas.map(s=>(<div className="sala-chip" key={s.id}><span className="swatch" style={{background:s.cor}}/><div className="info"><b>{s.nome}</b><span>{s.tipo}</span></div><button type="button" className="btn-danger" style={{padding:"8px 12px",fontSize:12}} onClick={()=>excluirSala(s)}>Excluir</button></div>))}
                  </div>

                  {/* BLOQUEIOS */}
                  <h4 style={{fontFamily:"Fraunces,serif",color:"var(--primary-dark)",fontSize:16,margin:"0 0 8px"}}>🚫 Horários Bloqueados Fixos</h4>
                  <p className="block-sub" style={{marginBottom:14}}>Bloqueios por dia da semana (recorrentes, toda semana).</p>
                  ...
                  <div className="form-grid">
                    ...
                    <div className="form-field full"><label>Descrição</label><input type="text" placeholder="Ex: Culto de domingo manhã" value={formBloqueio.descricao} onChange={e=>setFormBloqueio(f=>({...f,descricao:e.target.value}))}/></div>
                  ...
                  <div className="sala-chip-list" style={{marginTop:14}}>
                    ...
                    {bloqueios.map(b=>(<div className="sala-chip" key={b.id}><span className="swatch" style={{background:"#D6483A"}}/><div className="info"><b>{b.sala_nome} · {DIAS_SEMANA_FULL[b.dia_semana]} {b.hora_inicio}-{b.hora_fim}</b><span>{b.descricao||"Sem descrição"}</span></div><button type="button" className="btn-danger" style={{padding:"8px 12px",fontSize:12}} onClick={()=>excluirBloqueio(b.id)}>Remover</button></div>))}
                  </div>
                </div>
              )}
            </div>
          ...
          {!sucessoReserva?(
            <>
              ...
              <form className="form-grid" onSubmit={enviarReserva}>
                ...
                <div className="form-field"><label>Data</label><input type="text" disabled value={`${String(diaSelecionado).padStart(2,"0")}/${String(mesAtual+1).padStart(2,"0")}/${anoAtual}`}/></div>
                <div className="form-field"><label>Sala / Nave *</label>
                ...
                <div className="form-field full"><label>Observação (opcional)</label><input type="text" placeholder="Ex: Precisamos de 30 cadeiras" value={form.observacao} onChange={e=>setForm(f=>({...f,observacao:e.target.value}))}/></div>
                {salaTipo==="Nave"&&(
                  <div className="form-field full">
                    ...
                    <div style={{display:"flex",gap:20,marginTop:6,flexWrap:"wrap"}}>
                      <label style={{display:"flex",alignItems:"center",gap:8,fontWeight:600,fontSize:14,cursor:"pointer"}}>
                        <input type="checkbox" checked={form.precisaSom} onChange={e=>setForm(f=>({...f,precisaSom:e.target.checked}))} style={{width:18,height:18,accentColor:"var(--primary)",cursor:"pointer"}}/>🎤 Som
                      </label>
                      <label style={{display:"flex",alignItems:"center",gap:8,fontWeight:600,fontSize:14,cursor:"pointer"}}>
                        <input type="checkbox" checked={form.precisaProjecao} onChange={e=>setForm(f=>({...f,precisaProjecao:e.target.checked}))} style={{width:18,height:18,accentColor:"var(--primary)",cursor:"pointer"}}/>📽️ Projeção (Telão)
                      </label>
                    </div>
                  </div>
                )}
            ...
            <div style={{textAlign:"center",padding:"10px 0 4px"}}>
              ...
              <div style={{background:"var(--surface-soft)",borderRadius:12,padding:"14px 16px",margin:"12px 0",textAlign:"left",fontSize:13.5,lineHeight:1.7}}>
                ...
                {(sucessoReserva.precisa_som||sucessoReserva.precisa_projecao)&&<><b>Recursos:</b> {[sucessoReserva.precisa_som?"🎤 Som":null,sucessoReserva.precisa_projecao?"📽️ Projeção":null].filter(Boolean).join(" · ")}<br/></>}
              </div>
